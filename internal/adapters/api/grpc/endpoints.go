...
func (s *OrderGRPCServer) GetOrdersByStatus(req *pb.GetOrdersByStatusRequest, stream pb.OrderService_GetOrdersByStatusServer) error {
	orderCh, err := s.service.GetOrdersByStatus(stream.Context(), mapProtoStatusToCore(req.Status))
	if err != nil {
		return err
	}
