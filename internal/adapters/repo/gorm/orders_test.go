...
func TestOrderRepo_Create(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			savedOrder := Order{}
			err = dbConn.QueryRow(verifyQuery, tt.order.ID).Scan(
				&savedOrder.ID,
				&savedOrder.Items,
				&savedOrder.Status,
			...
			assert.Equal(t, tt.order.Status, core.Status(savedOrder.Status))
...
func TestOrderRepo_GetByStatus(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			orders, err := repo.GetByStatus(ctx, tt.status)
			if tt.expectError {
				require.Error(t, err)
				assert.Nil(t, orders)
				return
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.name == "non-existent order" {
				...
				require.NoError(t, err)
				assert.Equal(t, string(tt.newStatus), updated.Status)
			}
		})
	}
}
